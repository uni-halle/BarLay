    ...
    __global__ void find_best_swap_partner(
        ...
        for (; index < position_count; index += stride) {
            ...
            for (unsigned x = 0; x < layout::row_count; x++) {
                ...
                /******************************************************************************
                 * Load the barcodes in the first three columns of row x.
                 *****************************************************************************/

                if (x > 0)
                    S_xy[NORTH_EAST] = schedules[flat(x - 1, 0)];

                if (layout::col_count > 0)
                    S_xy[EAST] = schedules[flat(x, 0)];

                if (x + 1 < layout::row_count)
                    S_xy[SOUTH_EAST] = schedules[flat(x + 1, 0)];
                ...
                for (unsigned y = 0; y < layout::col_count; y++) {
                    ...
                    S_xy[NORTH_WEST] = S_xy[NORTH];
                    S_xy[WEST] = S_xy[CENTER];
                    S_xy[SOUTH_WEST] = S_xy[SOUTH];

                    S_xy[NORTH] = S_xy[NORTH_EAST];
                    S_xy[CENTER] = S_xy[EAST];
                    S_xy[SOUTH] = S_xy[SOUTH_EAST];
